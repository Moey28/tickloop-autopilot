import time, random, subprocess, sys, pathlib
import yaml
...
def jittered_backoff(i):
    ...
    if CFG['retries'].get('jitter', True):
        sleep = sleep * random.uniform(0.7, 1.3)
...
while True:
    ...
    if rc == 0:
        print("[autopilot] supervisor applied fix (or skipped for now). Retrying…", flush=True)
    else:
        print("[autopilot] supervisor had no fix this round. Will backoff and retry.", flush=True)
